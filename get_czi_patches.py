...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mask", "-m", required=True, help="Hdf5 file for all czi masks"
    )
    parser.add_argument(
        "--image",
        "-i",
    ...
    parser.add_argument(
        "--msifile", required=True, help="czi image file number i.e 1327,1900,2255 etc."
    )
    parser.add_argument("--patchsize", type=int, default=512, help="Size of patches")
    parser.add_argument("--path", "-p", required=True, help="path to save the patches")
    ...
    patches_image = np.squeeze(patches_image).reshape(
        patches_image.shape[0] * patches_image.shape[1], 512, 512, 3
    )
    print(patches_image.shape)

    # create patch for labels
    patches_labels = patchify(label, (512, 512), step=512)
    patches_labels = np.squeeze(patches_labels).reshape(
        patches_labels.shape[0] * patches_labels.shape[1], 512, 512
    )
