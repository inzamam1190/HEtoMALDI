...
def interpolation_msi(p, min_mz: int, max_mz: int, interpStep: float, filename: str):
    ...
    common_mz = (
        np.arange(start=int(min_mz), stop=int(max_mz), step=interpStep)
    ).astype(
        mz.dtype
    )  # common m/z values for interpolating the data

    f = h5py.File(filename, "w")
    mz_values = f.create_dataset(
        "common_mz", shape=common_mz.shape, dtype=common_mz.dtype
    )  # dataset for the common m/z

    # dataset for the intensity values
    intensity_values = f.create_dataset(
        "interpolated_intensities",
        shape=(
            len(common_mz),
            int(1 + p.imzmldict["max count of pixels x"]),
            int(1 + p.imzmldict["max count of pixels y"]),
        ),
        chunks=(common_mz.shape[0], 1, 1),
        dtype="f",
    )

    for i, (x, y, z) in tqdm(enumerate(p.coordinates), total=len(p.coordinates)):
        m_by_z, intensity = p.getspectrum(i)
        itest = scinterp.interp1d(
            m_by_z, intensity, fill_value=(0, 0), bounds_error=False
        )
...
if __name__ == "__main__":
    ...
    plt.plot(
        common_mz[:],
        intensity_data[:, p.coordinates[90][0], p.coordinates[90][1]],
