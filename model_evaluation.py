...
def get_prediction(patches, model, device):
    pred_mask = np.empty(
        shape=(patches.shape[0], patches.shape[1], 512, 512), dtype=np.float32
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-o",
        required=True,
        help="Where to place output torch tensor (should end in .pt or .pth)",
    )
    ...
    with h5py.File(ffile, "r") as f:
        ...
        with torch.no_grad():
            ...
            for i in tqdm(
                range(0, fullpred.shape[0] - args.patch_size + 1, args.patch_size)
            ):
                for j in range(
                    0, fullpred.shape[1] - args.patch_size + 1, args.patch_size
                ):
                    ...
                    fullpred[intstarty:intendy, intstartx:intendx,] = predprob[
