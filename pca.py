...
def pca(n_components, batch_size, xmin, xmax, ymin, ymax, intensity_data):
    ...
    with h5py.File(args.filename, "w") as f:
        pca = f.create_dataset(
            "data",
            shape=(n_components, intensity_data.shape[1], intensity_data.shape[2]),
            chunks=(n_components, 1, 1),
            dtype="f",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--filename", "-f", required="True", help="name of the saved pca file"
    )
