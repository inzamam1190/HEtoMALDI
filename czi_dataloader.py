...
class CZIDataModule(pl.LightningDataModule):
        ...
        self.transform = transforms.Compose(
            [  # transforms.ToPILImage(),
                ...
                transforms.Normalize(
                    mean=[696.5508, 479.1579, 525.0390],
    ...
    def setup(self):
        ...
        data_full = CZIPatchDataset(
            img_dir=self.data_dir, transform=self.transform
        )  # get the full dataset
        self.train, self.val, self.test = random_split(
            data_full, [5000, 1000, 1225], generator=torch.Generator().manual_seed(42)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datapath", "-d", required="True", help="data directory for the CZI patches"
    )
