...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        model.layer3.register_forward_hook(get_features("feats"))
        ...
        div = (M + 2 * H) // fts.shape[1]
        print(div)
        # downsample value

        if M % div != 0:
            raise RuntimeError(f"M % div = {M} % {div} = {M % div} != 0")
            ...
            with h5py.File(target_file, "a") as ftarget:
                ...
                for i in tqdm(range(tilenumy)):
                    for j in tqdm(range(tilenumx)):
                        ...
                        starty = max(i * M - H, 0)
                        startx = max(j * M - H, 0)
                        endy = min((i + 1) * M + H, ds_source.shape[1])
                        endx = min((j + 1) * M + H, ds_source.shape[2])

                        patch = torch.tensor(
                            # ds_source[:, M * i : M * (i + 1), M * j : M * (j + 1)]
                            ds_source[:, starty:endy, startx:endx],
                        ).cuda()
                        ...
                        # undo haloing to get interior region of features
                        if args.halo_width > 0:
                            startdivy = (i * M - starty) // div
                            startdivx = (j * M - startx) // div
                            enddivy = startdivy + M // div
                            enddivx = startdivx + M // div
                            ft = ft[..., startdivy:enddivy, startdivx:enddivx]
